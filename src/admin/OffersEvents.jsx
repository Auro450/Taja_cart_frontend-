...
function OffersEvents() {
  ...
  const fetchAnnouncements = async () => {
    try {
      const response = await fetch('http://192.168.0.112:3000/api/announcements');
      if (response.ok) {
  ...
  const fetchReviews = async () => {
    try {
      const response = await fetch('http://192.168.0.112:3000/api/reviews');
      if (response.ok) {
  ...
  const fetchBanners = async () => {
    try {
      const response = await fetch('http://192.168.0.112:3000/api/banners');
      if (response.ok) {
  ...
  const fetchHubs = async () => {
    try {
      const response = await fetch('http://192.168.0.112:3000/api/hubs');
      if (response.ok) {
  ...
  const handleSaveHub = async (e) => {
    e.preventDefault();
    const url = editingHub ? `http://192.168.0.112:3000/api/hubs/${editingHub.id}` : 'http://192.168.0.112:3000/api/hubs';
  ...
  const handleDeleteHub = async (id) => {
    if (window.confirm("Delete this hub?")) {
      try {
        const response = await fetch(`http://192.168.0.112:3000/api/hubs/${id}`, { method: 'DELETE' });
  ...
  const fetchSettings = async () => {
    try {
      const response = await fetch('http://192.168.0.112:3000/api/settings');
      if (response.ok) {
  ...
  const fetchOffers = async () => {
    try {
      const response = await fetch('http://192.168.0.112:3000/api/offers');
      if (response.ok) {
        const data = await response.json();
        setOffers(data);
      }
  ...
  const handleSaveOffer = async (e) => {
    e.preventDefault();
    const url = editingOffer ? `http://192.168.0.112:3000/api/offers/${editingOffer.id}` : 'http://192.168.0.112:3000/api/offers';
  ...
  const handleDeleteOffer = async (id) => {
    if (window.confirm("Delete this offer?")) {
      try {
        const response = await fetch(`http://192.168.0.112:3000/api/offers/${id}`, { method: 'DELETE' });
  ...
  const handleSaveAnnouncement = async (e) => {
    e.preventDefault();
    const url = editingAnnouncement ? `http://192.168.0.112:3000/api/announcements/${editingAnnouncement.id}` : 'http://192.168.0.112:3000/api/announcements';
  ...
  const handleDeleteAnnouncement = async (id) => {
    if (window.confirm("Delete this announcement?")) {
      try {
        const response = await fetch(`http://192.168.0.112:3000/api/announcements/${id}`, { method: 'DELETE' });
  ...
  const handleSaveReview = async (e) => {
    e.preventDefault();
    const url = editingReview ? `http://192.168.0.112:3000/api/reviews/${editingReview.id}` : 'http://192.168.0.112:3000/api/reviews';
  ...
  const handleToggleFeatured = async (review) => {
    try {
      const response = await fetch(`http://192.168.0.112:3000/api/reviews/${review.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteReview = async (id) => {
    if (window.confirm("Delete this review?")) {
      try {
        const response = await fetch(`http://192.168.0.112:3000/api/reviews/${id}`, { method: 'DELETE' });
        if (response.ok) fetchReviews();
      } catch (err) {
        console.error(err);
      }
  ...
  const handleToggleBannerApproved = async (banner) => {
    try {
      const response = await fetch(`http://192.168.0.112:3000/api/banners/${banner.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteBanner = async (id) => {
    if (window.confirm("Delete this banner?")) {
      try {
        const response = await fetch(`http://192.168.0.112:3000/api/banners/${id}`, { method: 'DELETE' });
        if (response.ok) fetchBanners();
      } catch (err) {
        console.error(err);
      }
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
                  {banners.map(banner => (
                    <div key={banner.id} style={{ border: '1px solid #cbd5e1', borderRadius: '12px', padding: '16px', backgroundColor: '#f8fafc', display: 'flex', flexDirection: 'column' }}>
                      <img src={`http://192.168.0.112:3000${banner.image}`} alt="Banner" style={{ width: '100%', height: '150px', objectFit: 'cover', borderRadius: '8px', marginBottom: '12px', border: '1px solid #e2e8f0' }} />
