...
function OffersEvents() {
  ...
  const handleSaveHub = async (e) => {
    e.preventDefault();
    const url = editingHub ? `http://localhost:3000/api/hubs/${editingHub.id}` : 'http://localhost:3000/api/hubs';
  ...
  const handleDeleteHub = async (id) => {
    if (window.confirm("Delete this hub?")) {
      try {
        const response = await fetch(`http://localhost:3000/api/hubs/${id}`, { method: 'DELETE' });
  ...
  const handleSaveOffer = async (e) => {
    e.preventDefault();
    const url = editingOffer ? `http://localhost:3000/api/offers/${editingOffer.id}` : 'http://localhost:3000/api/offers';
  ...
  const handleSaveAnnouncement = async (e) => {
    e.preventDefault();
    const url = editingAnnouncement ? `http://localhost:3000/api/announcements/${editingAnnouncement.id}` : 'http://localhost:3000/api/announcements';
  ...
  const handleSaveReview = async (e) => {
    e.preventDefault();
    const url = editingReview ? `http://localhost:3000/api/reviews/${editingReview.id}` : 'http://localhost:3000/api/reviews';
  ...
  const handleDeleteReview = async (id) => {
    if (window.confirm("Delete this review?")) {
      try {
        const response = await fetch(`http://localhost:3000/api/reviews/${id}`, { method: 'DELETE' });
        if (response.ok) fetchReviews();
      } catch (err) {
        console.error(err);
      }
  ...
  const handleDeleteBanner = async (id) => {
    if (window.confirm("Delete this banner?")) {
      try {
        const response = await fetch(`http://localhost:3000/api/banners/${id}`, { method: 'DELETE' });
        if (response.ok) fetchBanners();
      } catch (err) {
        console.error(err);
      }
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
                  {banners.map(banner => (
                    <div key={banner.id} style={{ border: '1px solid #cbd5e1', borderRadius: '12px', padding: '16px', backgroundColor: '#f8fafc', display: 'flex', flexDirection: 'column' }}>
                      <img src={`http://localhost:3000${banner.image}`} alt="Banner" style={{ width: '100%', height: '150px', objectFit: 'cover', borderRadius: '8px', marginBottom: '12px', border: '1px solid #e2e8f0' }} />
