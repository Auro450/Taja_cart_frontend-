...
function Store() {
  ...
  const fetchCategories = async () => {
    const res = await fetch('http://192.168.0.112:3000/api/categories');
  ...
  const fetchProducts = async () => {
    const res = await fetch('http://192.168.0.112:3000/api/products');
    const data = await res.json();
    setProducts(data);
    setLoading(false);
  };
  ...
  const fetchDeals = async () => {
    const res = await fetch('http://192.168.0.112:3000/api/deals');
    const data = await res.json();
    setDeals(data);
  };
  ...
  const handleSaveCategory = async (e) => {
    ...
    if (editingCategory.id) {
      await fetch(`http://192.168.0.112:3000/api/categories/${editingCategory.id}`, {
        method: 'PUT',
        body: formData
      });
    } else {
      await fetch(`http://192.168.0.112:3000/api/categories`, {
        method: 'POST',
        body: formData
      });
    }
  ...
  const handleDeleteCategory = async (id) => {
    if (window.confirm("Delete this category and all its products?")) {
      await fetch(`http://192.168.0.112:3000/api/categories/${id}`, { method: 'DELETE' });
      fetchCategories();
      fetchProducts();
  ...
  const handleSaveProduct = async (e) => {
    ...
    if (editingProduct.id) {
      await fetch(`http://192.168.0.112:3000/api/products/${editingProduct.id}`, {
        method: 'PUT',
        body: formData
      });
    } else {
      await fetch(`http://192.168.0.112:3000/api/products`, {
        method: 'POST',
        body: formData
      });
    }
  ...
  const handleDeleteProduct = async (id) => {
    if (window.confirm("Delete this product?")) {
      await fetch(`http://192.168.0.112:3000/api/products/${id}`, { method: 'DELETE' });
      fetchProducts();
    }
  };
  ...
  const handleSaveDeal = async (e) => {
    ...
    if (editingDeal.id) {
      await fetch(`http://192.168.0.112:3000/api/deals/${editingDeal.id}`, {
        method: 'PUT',
        body: formData
      });
    } else {
  ...
  const handleDeleteDeal = async (id) => {
    if (window.confirm("Delete this deal?")) {
      await fetch(`http://192.168.0.112:3000/api/deals/${id}`, { method: 'DELETE' });
      fetchDeals();
    }
  };
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px' }}>
                  {deals.map(p => (
                    <div key={p.id} style={{ border: '1px solid #fde047', borderRadius: '8px', padding: '12px', display: 'flex', flexDirection: 'column', backgroundColor: '#fefce8' }}>
                      <img src={p.image?.startsWith('/uploads') ? `http://192.168.0.112:3000${p.image}` : p.image} alt={p.name} style={{ width: '100%', height: '100px', objectFit: 'contain', marginBottom: '12px' }} />
