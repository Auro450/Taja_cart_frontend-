...
function Notifications() {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await fetch('http://192.168.0.112:3000/api/admin/notifications');
  ...
  const handleSave = async (e) => {
    ...
    try {
      const url = editingId 
        ? `http://192.168.0.112:3000/api/admin/notifications/${editingId}`
        : 'http://192.168.0.112:3000/api/admin/notifications';
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this notification?')) {
      try {
        const response = await fetch(`http://192.168.0.112:3000/api/admin/notifications/${id}`, {
          method: 'DELETE',
        });
        if (response.ok) {
          fetchNotifications();
  ...
  const handleToggleActive = async (notification) => {
    try {
      const response = await fetch(`http://192.168.0.112:3000/api/admin/notifications/${notification.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
